...
# Handle both possible shap output formats (list vs 3D array) across shap versions
...
# ── DNN: KernelExplainer ──────────────────────────────────────────────────
print("Computing SHAP values for DNN (slower, ~1-3 min)...")
...
# ── Save raw SHAP values for reuse (e.g. dependence plots later) ─────────
...
plt.tight_layout()
plt.savefig("outputs/xai/tabular/shap/shap_beeswarm_rf_newww.png", dpi=150, bbox_inches="tight")
plt.close()
